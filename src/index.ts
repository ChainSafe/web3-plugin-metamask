...
export default class MetamaskPlugin extends Web3PluginBase {
    ...
    public async requestPermissions(requestPermissionsObject: Record<string, any>):Promise<Permission[]> {
        return await this.requestManager.send({method:'wallet_requestPermissions', params:[requestPermissionsObject]});
    }


    public async revokePermissions(permission: Record<string,any>) {
    ...
    public async requestSnap(requestSnap: Record<string, any>): Promise<Snap> {
      return await this.requestManager.send({method:'wallet_snap', params:[requestSnap]});
  }

    public async snap(snapParameter: {snapId: string, request: {method: string, params: Record<string, any>}}): Promise<Snap> {
        return await this.requestManager.send({method:'wallet_snap', params:[snapParameter]});
    }
}
